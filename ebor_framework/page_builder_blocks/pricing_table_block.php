	...
	class AQ_Pricing_Table_Block extends AQ_Block {
		...
		function form($instance) {
			...
			<p class="description note">
				<?php _e('Use this block to add pricing tables to your page.', 'other') ?>
			</p>
			
			<p class="description">
				<label for="<?php echo $this->get_field_id('title') ?>">
					<?php _e('Title for this Table', 'other'); ?>
					<?php echo aq_field_input('title', $block_id, $title, $size = 'full') ?>
				</label>
			</p>
			
			<p class="description fourth">
				<label for="<?php echo $this->get_field_id('colour') ?>">
					Table colour<br/>
					<?php echo aq_field_select('colour', $block_id, $colour_options, $colour, $block_id); ?>
				</label>
			</p>
			
			<p class="description">
				<label for="<?php echo $this->get_field_id('currency') ?>">
					<?php _e('Currency', 'other'); ?>
			...
			<div class="description cf">
					...
					<p></p>
					<a href="#" rel="table" class="aq-sortable-add-new button"><?php _e('Add New', 'other'); ?></a>
					<p></p>
				</div>
				
			<p class="description">
				<label for="<?php echo $this->get_field_id('button_link') ?>">
					<?php _e('Button Link', 'other'); ?>
				...
				<li id="<?php echo $this->get_field_id('tabs') ?>-sortable-item-<?php echo $count ?>" class="sortable-item" rel="<?php echo $count ?>">
					...
					<div class="sortable-head cf">
						...
						<div class="sortable-handle">
							<a href="#"><?php _e('Open / Close', 'other'); ?></a>
						</div>
					</div>
					...
					<div class="sortable-body">
						...
						<p class="tab-desc description"><a href="#" class="sortable-delete"><?php _e('Delete','other'); ?></a></p>
					</div>
		...
		function block($instance) {
			...
			echo '<div class="' . $size . ' pricing-table">
				  <h3 class="header">' . htmlspecialchars_decode($title) . '</h3>
				  <span class="price button '.$colour.'">'. $currency . $price . '</span>
				  <ul>';
			foreach($tabs as $tab){
				echo '<li>'.$tab['title'].'</li>';
			}
			echo '</ul>
				  <a href="'.esc_url($button_link).'" class="button '.$colour.'">'.htmlspecialchars_decode($button_text).'</a>
			      </div>';
